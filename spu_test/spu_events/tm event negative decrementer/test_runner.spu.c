...
int main(uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4)
{
  ...
  //volatile char failedBuf[8192] __attribute__((aligned(128))) = {0};
    ...
    spu_printf("waiting n seconds\n");
    spu_writech(SPU_WrDec, 100000000);
    ...
    spu_writech(SPU_WrDec, spu_readch(SPU_RdDec)); // Rewrite the negative decrementer value to possibly trigger an event
    while (spu_readchcnt(SPU_RdEventStat) == 0){}
