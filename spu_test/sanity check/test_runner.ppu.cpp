...
int main(void)
{
    ...
    void* fill = (void*)((uint32_t)(malloc(1 << 16)) & ~127); // align the address manually
    printf("cell is %x\n",sys_spu_thread_write_snr(thr_id, 0 ,(uint32_t)(fill)));

    asm volatile ("nop;eieio;sync");
    printf("%x\n", uint32_t(fill) & ~127);
    uint64_t fuck_reservation = 0x1;
    while(sys_spu_thread_get_exit_status(thr_id, &ret) == ESTAT) // checks if thread has exit via the error code
    {
        *(uint64_t*)fill = fuck_reservation++; // never write the same value again, to make sure it works on rpcs3
        asm volatile ("nop;eieio;sync");
    }
    /*int cause;
    int status;
