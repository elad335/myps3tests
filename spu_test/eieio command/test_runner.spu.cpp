...
int main(uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4)
{
    ...
    spu_writech(MFC_Cmd, 0xC8); // EIEIO command
    sync();
    ...
    spu_writech(MFC_WrTagMask, 0x2); // EIEIO command's tag
    spu_writech(MFC_WrTagUpdate, 2); // Wait until EIEIO command has finished (according to the tag status)
    sync(); sync(); sync(); sync();
    spu_printf("the tag status is : %x \n", spu_readch(MFC_RdTagStat)); // result: stalled, shows the eieio cmd acts like a full barrier
