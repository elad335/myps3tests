...
int main(void)
{
	...
	ret = sys_spu_thread_group_create(&grp_id, 2, 100, &grp_attr);
	if (ret != CELL_OK) {
	...
	{
		static const int thread_number = 0;
		sys_spu_thread_argument_t thr_args;
		thr_args.arg1 = thread_number; 
		ret = sys_spu_thread_initialize(&thr_id, grp_id, 255, &img, &thr_attr, &thr_args);
		if (ret != CELL_OK) {
	...
	{
		static const int thread_number = 255;
		sys_spu_thread_argument_t thr_args;
		thr_args.arg1 = thread_number; 
