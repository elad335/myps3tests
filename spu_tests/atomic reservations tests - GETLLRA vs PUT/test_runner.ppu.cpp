...
int main(void)
{
	...
	ENSURE_OK(sys_spu_thread_group_create(&grp_id, 5, prio, &grp_attr));
	...
	thr_args.arg2 = 2;
	ENSURE_OK(sys_spu_thread_initialize(&thr_id, grp_id, 2, &img, &thr_attr, &thr_args));
	thr_args.arg2 = 3;
	ENSURE_OK(sys_spu_thread_initialize(&thr_id, grp_id, 3, &img, &thr_attr, &thr_args));
	thr_args.arg2 = 4;
	ENSURE_OK(sys_spu_thread_initialize(&thr_id, grp_id, 4, &img, &thr_attr, &thr_args));
