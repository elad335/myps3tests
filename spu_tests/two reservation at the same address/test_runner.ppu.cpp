...
int main(void)
{
    ...
    sys_spu_thread_t thr_id2;
    sys_spu_thread_attribute_t thr_attr2;

    sys_spu_thread_attribute_initialize(thr_attr2);
    sys_spu_thread_attribute_name(thr_attr2, "test spu thread2");
    sys_spu_thread_argument_t thr_args2;
    ret = sys_spu_thread_initialize(&thr_id2, grp_id, 1, &img, &thr_attr2, &thr_args2);
    if (ret != CELL_OK) {
        printf("sys_spu_thread_initialize: %d\n", ret);
        return ret;
    }
    ...
    printf("cell is %x\n",sys_spu_thread_write_snr(thr_id2, 0 ,(uint32_t)(fill) & ~127)); // align the address manually
