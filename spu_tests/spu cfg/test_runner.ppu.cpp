...
int main(void)
{
	...
	g_ec = sys_spu_thread_set_spu_cfg(-1u /* No spu thread */, 4 /*Invalid value*/);
	...
	// Check value after group_join
	uint64_t value;
	sys_spu_thread_get_spu_cfg(thr_id, &value);
	printf("sys_spu_thread_get_spu_cfg value after group_join: 0x%x\n", value);
