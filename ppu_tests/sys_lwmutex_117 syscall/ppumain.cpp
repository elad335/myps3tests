...
int main() 
{
	...
	ENSURE_OK(sys_lwmutex_create(&lwmutex, &mutex_attr));
	ENSURE_OK(sys_lwcond_create(&lwcond, &lwmutex, &cond_attr));

	ENSURE_OK(lv2_lwmutex_117(&lwmutex));
	...
	ENSURE_OK(lv2_lwmutex_117(&lwmutex));
	ENSURE_OK(lv2_lwmutex_unlock(&lwmutex));
	...
	ENSURE_OK(sys_ppu_thread_create(&waiter_tid, &waiter, 0, 1001, 0x10000, 1, "Waiter"));
