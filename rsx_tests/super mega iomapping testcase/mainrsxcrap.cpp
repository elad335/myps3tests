...
int main() {
	...
	sys_mmapper_map_memory(u64(addr1) + 9<<20, mem_id, SYS_MEMORY_PROT_READ_WRITE);
	...
	{
		system_call_5(SYS_RSX_CONTEXT_IOMAP, context_id, (257<<20), u64(addr1) + (3<<20), 0x00200000 /*size = 2mb*/, 0xe000000000000800ull);
		printf("sys_rsx_context_iomap3: ret is 0x%x\n", ret);
	}


	{
		system_call_3(SYS_RSX_CONTEXT_IOUNMAP, context_id, 0x000000, 0x00200000 /*size = 2mb*/);
		printf("sys_rsx_context_iounma12: ret is 0x%x\n", ret);
	}

	{
		system_call_3(SYS_RSX_CONTEXT_IOUNMAP, context_id, 0x000000, 0x00200000 /*size = 2mb*/);
		printf("sys_rsx_context_iounmap2: ret is 0x%x\n", ret);
	}

	{
		system_call_3(SYS_RSX_CONTEXT_IOUNMAP, context_id, 23<<20, 0x00200000 /*size = 2mb*/);
		printf("sys_rsx_context_iounmap3: ret is 0x%x\n", ret);
	}
