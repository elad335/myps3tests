	...
	uint32_t count, uint8_t type, uint8_t location, uint32_t indicies)
{
	...
	// Invalid primitive!!
	CELL_GCM_METHOD_SET_BEGIN_END(CELL_GCM_CURRENT, 
		CELL_GCM_COMMAND_CAST(0xff));

	// If we return here realhw does not crash
...
#endif
cellGcm_SetDrawIndexArray(&Gcm, CELL_GCM_PRIMITIVE_TRIANGLE_STRIP, 4,type,CELL_GCM_LOCATION_LOCAL, offs);
