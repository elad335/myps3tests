...
public:
	...
	{
		*OffsetToAddr(begin + addr) = cmd;
	}

	inline void push(u32 cmd)
	{
		*OffsetToAddr(current) = cmd;	
		current += 4;
	}

	inline void push(u32 count, u32 command)
	{
		*OffsetToAddr(current) = (count << 18) | (command << 2);	
		current += 4;
	}
};
...
int main() {
	...
	*OffsetToAddr(ctrl->get) = (1<<20) | RSX_METHOD_NEW_JUMP_CMD;
	...
	c.push(1);
	c.push(c.current | RSX_METHOD_OLD_JUMP_CMD);
