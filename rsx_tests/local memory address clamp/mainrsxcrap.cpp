...
int main() {
	...
	cellGcmGetCurrentDisplayBufferId(&id);
 	...
 	cellGcmSetDisplayBuffer(id, (1<<31) + (2<<20), 1280*4, 1280, 720);
	printf("ret is 0x%x\n", ret);
	...
	*OffsetToAddr(ctrl->get) = (1<<20) | RSX_METHOD_NEW_JUMP_CMD;
	...
	cellGcmSetDrawBegin(&Gcm, CELL_GCM_PRIMITIVE_POINTS);

	asm volatile ("sync;eieio");

	cellGcmSetFlip(&Gcm, id);
	c.bindJmp(flipForever);
	...
	u32* loc = ptr_caste(gcmDisplay,u32);
	while (int_cast(loc) < (gcmDisplay + (1280*720*4)))
	{
		*loc = rand();
		loc += u32(rand()) & 0x3f;
