...
#define int_cast(addr) reinterpret_cast<uint32_t>(addr)
...
int main() {
	...
	CellGcmOffsetTable RSXIOMem;
	cellGcmGetOffsetTable(&RSXIOMem);

	printf("offset_io=0x%x, offset_ea=0x%x\n", int_cast(RSXIOMem.ioAddress), int_cast(RSXIOMem.eaAddress));
	cellGcmIoOffsetToAddress(0, &offset);
	printf("offset=0x%x", *(u16*)offset);
