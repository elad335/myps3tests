...
int main() {
	...
	*OffsetToAddr(ctrl->get) = (1<<20) | RSX_METHOD_NEW_JUMP_CMD;
	...
	struct VertexData3D { uint16_t i[6];} __attribute__ ((aligned(2)));

#define MINX -1
#define MINY -1
#define MAXX 1
#define MAXY 1
	// (65535 / 2) is conversation to S1 vertex type
	static VertexData3D vertices[4] = {
		  // vertex           tex coord
		{ (MINX) * (65535 / 2),(MINY) * (65535 / 2),0, 1 * (65535 / 2), 0, 0 },
		{ (MAXX) * (65535 / 2),(MINY) * (65535 / 2),0, (1) * (65535 / 2), cellGcmGetFloatToHalf(1) * (65535 / 2), 0 },
		{ (MINX) * (65535 / 2),(MAXY) * (65535 / 2),0, (1) * (65535 / 2), cellGcmGetFloatToHalf(0) * (65535 / 2), (1) * (65535 / 2) },
		{ (MAXX) * (65535 / 2),(MAXY) * (65535 / 2),0, (1) * (65535 / 2), cellGcmGetFloatToHalf(1) * (65535 / 2), (1) * (65535 / 2) }
	};
#undef MINX
#undef MINY
#undef MAXX
#undef MAXY
	...
	cellGcmSetVertexDataArray(&Gcm, g_obj_coord_idx, 0, sizeof(VertexData3D), 4, 
	                      8 /*illegal vertex type!*/, CELL_GCM_LOCATION_LOCAL, 
						  0x0A00000u ); 
