...
int main() {
	...
	*OffsetToAddr(ctrl->get) = (1<<20) | RSX_METHOD_NEW_JUMP_CMD;
	...
	c.push(0x40080050); // SET_REFERENCE, count equals 2, non-increment cmd
	c.push(0);
	gcmLabel end = c.newLabel(); // Attempt to stop execution in the middle of the command
	c.push(1);
	c.bindJmp(c.newLabel()); // Jump to self
	...
	// Wait for complition (with timeout of 5s)
	u32 start = 0;
	while(ctrl->ref != 1 && start++ < 5)
	{
		sys_timer_sleep(1);
	}

	printf("Ref=0x%x, Get=0x%x, Put=0x%x. \nsample finished.\n", ctrl->ref, ctrl->get, ctrl->put);
