...
#undef MAXY
	...
	const u32 vertex_input_mask = (1u << g_obj_coord_idx) | (1u << g_tex_coord_idx); 
	cellGcmSetVertexAttribInputMask(&Gcm, vertex_input_mask); 

	cellGcmSetInvalidateTextureCache(&Gcm, CELL_GCM_INVALIDATE_TEXTURE);
	cellGcmSetInvalidateVertexCache(&Gcm);

	//cellGcmSetDrawInlineArray(&Gcm, CELL_GCM_PRIMITIVE_TRIANGLE_STRIP, 20, &vertices);
	cellGcmSetDrawBegin(&Gcm, CELL_GCM_PRIMITIVE_TRIANGLE_STRIP);
	{
		gcmLabel here = c.newLabel(); 
		{ c.push(1, NV4097_DRAW_ARRAYS); c.push(0x3000001); }
		{ c.push(1, NV4097_DRAW_ARRAYS); c.push(0x3000000); }
	}
	cellGcmSetDrawEnd(&Gcm);
	//cellGcmSetDrawArrays(&Gcm, CELL_GCM_PRIMITIVE_TRIANGLE_STRIP, 1, 4);
	gfxFence(&Gcm);
