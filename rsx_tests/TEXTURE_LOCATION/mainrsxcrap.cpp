...
#undef MAXY
	...
	// Texture attributes set earlier, but this time with an invalid location (lowest two bits)
	c.push(1, NV4097_SET_TEXTURE_FORMAT + (0 * 8)); c.push((0x1a52a & ~3) | 3);
	...
	while (true || load_vol(ctrl->ref) != 2) sys_timer_usleep(1000);
