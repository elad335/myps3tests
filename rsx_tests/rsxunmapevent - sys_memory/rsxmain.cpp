...
int main() {
	...
	printf("alloc\n");
	sys_memory_allocate(1<<20, SYS_MEMORY_PAGE_SIZE_1M, &addr1);
	if(result != CELL_OK) {
        printf("%s %d : %x\n", __FUNCTION__, __LINE__, result);
        exit(result);
    }
	...
	printf("iomap\n");
	{
		...
		system_call_5(SYS_RSX_CONTEXT_IOMAP, context_id, 0 << 20, u64(addr1), 0x1 << 20, flags);
		int ret = (int)(p1);
		printf("ret is 0x%x\n", ret);
	}
	sys_timer_sleep(1);
